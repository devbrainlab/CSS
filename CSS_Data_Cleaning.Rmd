...
load("/Volumes/devbrainlab/Covid19SocialScreens/CSS_Data/Standardized_Analysis_Data.Rda")
...
analysis_data <- processed_data %>%
  ...
  select(1:13, 194:206, 221:228)
save(analysis_data, file="analysis_data.Rda")


#Subjective Screen Time
processed_data$tech_use_1 <- as.numeric(processed_data$tech_use_1)
processed_data$tech_use_2 <- as.numeric(processed_data$tech_use_2)
processed_data$tech_use_3 <- as.numeric(processed_data$tech_use_3)
processed_data$tech_use_4 <- as.numeric(processed_data$tech_use_4)
processed_data <- processed_data %>% mutate(subj_screen_total = rowSums(.[c("tech_use_1", "tech_use_2", "tech_use_3", "tech_use_4")], na.rm = T))
...
data = analysis_data, geom = "line", main= "Anxiety Across Waves (per participant)",xlab="Wave", ylab="YAM Score") + theme_classic() + theme(legend.position="none")
ggsave("AnxietyParticipants.png")

qplot(x=wave, y=mspss_total, colour = factor(record_id),
data = analysis_data, geom = "line", main="Social Support Across Waves (per participant)", xlab="wave", ylab="Social Support") + theme_classic() + theme(legend.position="none")
ggsave("SocialSupportParticipants.png")

qplot(x=wave, y=prop_friend_stability, colour = factor(record_id),
data = subset(analysis_data, wave!=1), geom = "line", main="Friend Stability Across Waves (per participant)", xlab="Wave", ylab="Friend Stability") + theme_classic() + theme(legend.position="none")
ggsave("FriendStabilityParticipants_Subset.png")

qplot(x=wave, y=final_social_min, colour = factor(record_id),
data = analysis_data, geom = "line", main="Social Screen Minutes Across Waves (per participant)", xlab="Wave", ylab="Social Screen Minutes") + theme_classic() + theme(legend.position="none")
ggsave("SocialMinutesParticipants.png")

qplot(x=wave, y=final_total_min, colour = factor(record_id),
data = analysis_data, geom = "line", main="Total Screen Minutes Across Waves (per participant)", xlab="Wave", ylab="Total Screen Minutes") + theme_classic() + theme(legend.position="none")
ggsave("TotalMinutesParticipants.png")

qplot(x=wave, y=final_social_prop, colour = factor(record_id),
data = analysis_data, geom = "line", main="Social Screen Use Proportion Across Waves (per participant)", xlab="Wave", ylab="Social Screen Use Proportion") + theme_classic() + theme(legend.position="none")
...
mod_h1_between <- lme(yam_total ~ final_total_min,
              random = ~1|record_id,
              data=(yam_analysis %>% filter(!is.na(final_social_min))),
              method="ML",
              na.action=na.omit)
...
mod_h1_soc_month <- lme(yam_total_cw2 ~ final_total_min_cw2 + final_social_min_cw2 + StartMonth,
                  random = ~1|record_id,
                  data=(yam_analysis %>% filter(!is.na(final_social_min_cw2))),
                  method="ML",
                  na.action=na.omit)
anova(mod_h1_soc, mod_h1_soc_month)

mod_h1_between_soc <- lme(yam_total ~ final_total_min + final_social_min,
              random = ~1|record_id,
              data=(yam_analysis %>% filter(!is.na(final_social_min))),
              method="ML",
              na.action=na.omit)
anova(mod_h1_between,mod_h1_between_soc)

#Add month to the model
mod_h1_month <- lme(yam_total_cw2 ~ final_total_min_cw2 + StartMonth,
                    random = ~1|record_id,
                    data=(yam_analysis %>% filter(!is.na(final_social_min_cw2))),
                  method="ML",
                  na.action=na.omit)
anova(mod_h1, mod_h1_month)

mod_h1_month_only <- lme(yam_total_cw2 ~ StartMonth,
                    random = ~1|record_id,
                    data=(yam_analysis %>% filter(!is.na(final_social_min_cw2)&!is.na(final_total_min_cw2))),
                  method="ML",
                  na.action=na.omit)
anova(mod_h1_month, mod_h1_month_only)

mod_h1_between_month <- lme(yam_total ~ final_total_min + StartMonth,
              random = ~1|record_id,
              data=(yam_analysis %>% filter(!is.na(final_social_min))),
              method="ML",
              na.action=na.omit)
anova(mod_h1_between,mod_h1_between_month)
...
mod_h1_prop_month <- lme(yam_total_cw2 ~ final_social_prop_cw2 + StartMonth,
              random = ~1|record_id,
              data=yam_analysis,
              method="ML",
              na.action=na.omit)
anova(mod_h1_prop,mod_h1_prop_month)
...
mod_h2 <- lme(mspss_total_cw2 ~ final_total_min_cw2,
              random = ~1|record_id,
              data=(mspss_analysis %>% filter(!is.na(final_social_min_cw2))),
              method="ML",
              na.action=na.omit)

mod_h2_between <- lme(mspss_total ~ final_total_min,
              random = ~1|record_id,
...
#Add month
mod_h2_month <- lme(mspss_total_cw2 ~ final_total_min_cw2 + StartMonth,
              random = ~1|record_id,
              data=(mspss_analysis %>% filter(!is.na(final_social_min_cw2))),
              method="ML",
              na.action=na.omit)
anova(mod_h2, mod_h2_month)
...
mod_h2_prop_month <- lme(mspss_total_cw2 ~ final_social_prop_cw2 + StartMonth,
              random = ~1|record_id,
              data=mspss_analysis,
              method="ML",
              na.action=na.omit)
anova(mod_h2_prop,mod_h2_prop_month)
...
mod_h3_tot <- lme(final_social_min_cw2 ~ final_total_min_cw2,
              random = ~1|record_id,
              data=filter(standardized_analysis_data, !is.na(prop_friend_stability)),
              method="ML",
              na.action=na.omit)

mod_h3_both <- lme(final_social_min_cw2 ~ final_total_min_cw2 + prop_friend_stability,
              random = ~1|record_id,
              data=standardized_analysis_data,
              method="ML",
              na.action=na.omit)
anova(mod_h3_tot,mod_h3_both)

mod_h3_both_month <- lme(final_social_min_cw2 ~ final_total_min_cw2 + prop_friend_stability_cw2 + StartMonth,
              random = ~1|record_id,
              data=standardized_analysis_data,
              method="ML",
              na.action=na.omit)
anova(mod_h3_both,mod_h3_both_month)

mod_h3_prop <- lme(final_social_prop_cw2 ~ prop_friend_stability_cw2,
              random = ~1|record_id,
              data=standardized_analysis_data,
              method="ML",
              na.action=na.omit)

mod_h3_prop_month <- lme(final_social_prop_cw2 ~ prop_friend_stability_cw2 + StartMonth,
              random = ~1|record_id,
              data=standardized_analysis_data,
              method="ML",
              na.action=na.omit)
anova(mod_h3_prop,mod_h3_prop_month)

#Add month
mod_h3_tot_month <-  lme(final_social_min_cw2 ~ final_total_min_cw2 +StartMonth,
              random = ~1|record_id,
              data=filter(standardized_analysis_data, !is.na(prop_friend_stability_cw2)),
              method="ML",
              na.action=na.omit)
anova(mod_h3_tot,mod_h3_tot_month)
...
mod_h3_tot_between <- lme(final_social_min ~ final_total_min,
              random = ~1|record_id,
              data=filter(analysis_data, !is.na(prop_friend_stability)),
              method="ML",
              na.action=na.omit)

mod_h3_both_between <- lme(final_social_min ~ final_total_min + prop_friend_stability,
              random = ~1|record_id,
              data=analysis_data,
              method="ML",
              na.action=na.omit)
anova(mod_h3_tot_between,mod_h3_both_between)

mod_h3_prop_between <- lme(final_social_prop ~ prop_friend_stability,
              random = ~1|record_id,
              data=analysis_data,
              method="ML",
              na.action=na.omit)
...
Models w/ Subjective Screen Use
```{r}
#Between Participants
mod_subjective_objective <- lme(final_total_min ~ subj_screen_total,
              random = ~1|record_id,
              data=(analysis_data %>% filter(!is.na(final_total_min))),
              method="ML",
              na.action=na.omit)
ggplot(analysis_data, aes(x=final_total_min, y=subj_screen_total)) + geom_point()
ggplot(analysis_data, aes(x=final_social_min, y=tech_use_3)) + geom_point()


mod_h1_subjective_total <- lme(yam_total ~ subj_screen_total,
              random = ~1|record_id,
              data=(yam_analysis %>% filter(!is.na(subj_screen_total)& !is.na(tech_use_3))),
              method="ML",
              na.action=na.omit)

mod_h1_subjective_social <- lme(yam_total ~ subj_screen_total + tech_use_3,
              random = ~1|record_id,
              data=(yam_analysis %>% filter(!is.na(subj_screen_total))),
              method="ML",
              na.action=na.omit)
anova(mod_h1_subjective_total,mod_h1_subjective_social)


#Within Participants
mod_h1_subjective_within_total <- lme(yam_total_cw2 ~ subj_screen_total_cw2,
              random = ~1|record_id,
              data=(yam_analysis %>% filter(!is.na(subj_screen_total_cw2)& !is.na(tech_use_3_cw2))),
              method="ML",
              na.action=na.omit)

mod_h1_subjective_within_social <- lme(yam_total_cw2 ~ subj_screen_total_cw2 + tech_use_3_cw2,
              random = ~1|record_id,
              data=(yam_analysis %>% filter(!is.na(subj_screen_total_cw2))),
              method="ML",
              na.action=na.omit)
anova(mod_h1_subjective_total,mod_h1_subjective_social)
```
...
#Model H1 (Between)
total_minutes_between<-round(seq(min(yam_analysis$final_total_min,na.rm = TRUE),
                                    max(yam_analysis$final_total_min,na.rm = TRUE),by=500),2)
                 data.pred = data.frame(final_total_min=total_minutes_between)
                 y.pred = predict(mod_h1_between,
                     newdata=data.pred,se=T,level = 0)
                 data.pred = cbind.data.frame(data.pred,
                                              y.pred)
                 scale = 1.96
                 #alternate option
                 scale = 1.96
                  designmat<-model.matrix(eval(eval(mod_h1_between$call$fixed)[-2]),
                        data.pred[-3]) #make design matrix
                  SDvalue<-sqrt(diag(designmat %*% mod_h1_between$varFix %*% t(designmat))) #calculate standard deviation for each point for each model
                  y.lower<-y.pred-(scale*SDvalue) #calculate confidence intervals - lower
                  y.upper<-y.pred+(scale*SDvalue) #calculate confidence intervals - upper
                  data.pred = cbind.data.frame(data.pred,
                             y.lower,
                          ...
                          #geom_line(data=yam_analysis, #raw data
                             #       aes(x=final_total_min_cw2, y=yam_total_cw2,group=record_id,
                               #         colour=record_id),
                                ...
                                #    size=.6) +
                          geom_point(data=yam_analysis,
                                     aes(x=final_total_min, y=yam_total,group=record_id,colour=wave),
                                     size=1.5,
                                     alpha=0.5) +
                          geom_line(data=data.pred, #predicted data
                                    aes(x=final_total_min, y=y.pred), #y.pred as x?
                                    size=1,
                                    colour="purple2")+
                          geom_ribbon(data=data.pred, #predicted data
                                      aes(x=final_total_min, y=y.pred,ymin=y.lower,
                                          ymax=y.upper),
                                      alpha=0.4,
                                      fill="purple2")+
                          theme_kate()

                   ggsave(filename="ModH1_Between.png",
                          width=6, height=4, units='in', dpi=300)
                   
#Model H1 (Proportional)
total_minutes_prop<-round(seq(min(yam_analysis$final_social_prop_cw2,na.rm = TRUE),
                                    max(yam_analysis$final_social_prop_cw2,na.rm = TRUE),by=.1),2)
                 data.pred = data.frame(final_social_prop_cw2=total_minutes_prop)
                 y.pred = predict(mod_h1_prop,
                     newdata=data.pred,se=T,level = 0)
                 data.pred = cbind.data.frame(data.pred,
                                              y.pred)
                 scale = 1.96
                 #alternate option
                 scale = 1.96
                  designmat<-model.matrix(eval(eval(mod_h1_prop$call$fixed)[-2]),
                        data.pred[-3]) #make design matrix
                  SDvalue<-sqrt(diag(designmat %*% mod_h1_prop$varFix %*% t(designmat))) #calculate standard deviation for each point for each model
                  y.lower<-y.pred-(scale*SDvalue) #calculate confidence intervals - lower
                  y.upper<-y.pred+(scale*SDvalue) #calculate confidence intervals - upper
                  data.pred = cbind.data.frame(data.pred,
                             y.lower,
                ...
                data.pred$ypred<-data.pred$y.pred
                        ...
                        ggplot(data=NULL
                          ...
                          ylab("Predicted Anxiety")+
                          xlab("Social Use Proportion")+
                          ggtitle("Model H1 (Prop)")+
                          geom_point(data=yam_analysis,
                                     aes(x=final_social_prop_cw2, y=yam_total_cw2,group=record_id,colour=wave),
                                     size=1.5,
                                     alpha=0.5) +
                          geom_line(data=data.pred, #predicted data
                                    aes(x=final_social_prop_cw2, y=y.pred), #y.pred as x?
                                    size=1,
                                    colour="purple2")+
                          geom_ribbon(data=data.pred, #predicted data
                                      aes(x=final_social_prop_cw2, y=y.pred,ymin=y.lower,
                                          ymax=y.upper),
                                      alpha=0.4,
                                      fill="purple2")+
                          theme_kate()

                   ggsave(filename="ModH1_Prop.png",
                          width=6, height=4, units='in', dpi=300)
...
#Model H2
 total_minutes_h2<-round(seq(min(mspss_analysis$final_total_min_cw2,na.rm = TRUE),
                                    max(mspss_analysis$final_total_min_cw2,na.rm = TRUE),by=400),2)
                 data.pred = data.frame(final_total_min_cw2=total_minutes_h2)
                 y.pred = predict(mod_h2,
                     newdata=data.pred,se=T,level = 0)
                 data.pred = cbind.data.frame(data.pred,
                                              y.pred)
                 #alternate option
                 scale = 1.96
                  designmat<-model.matrix(eval(eval(mod_h2$call$fixed)[-2]),
                        data.pred[-3]) #make design matrix
                  SDvalue<-sqrt(diag(designmat %*% mod_h2$varFix %*% t(designmat))) #calculate standard deviation for each point for each model
                  y.lower<-y.pred-(scale*SDvalue) #calculate confidence intervals - lower
                  y.upper<-y.pred+(scale*SDvalue) #calculate confidence intervals - upper
                  data.pred = cbind.data.frame(data.pred,
                             y.lower,
                ...
                data.pred$ypred<-data.pred$y.pred
                        ...
                        ggplot(data=NULL
                          ...
                          ylab("Predicted Social Support")+
                          xlab("Total Screen Use")+
                          ggtitle("Model H2 (Within)")+
                          geom_point(data=mspss_analysis,
                                     aes(x=final_total_min_cw2, y=mspss_total_cw2,group=record_id,colour=wave),
                                     size=1.5,
                                     alpha=0.5) +
                          geom_line(data=data.pred, #predicted data
                                    aes(x=final_total_min_cw2, y=y.pred), 
                                    size=1,
                                    colour="purple2")+
                          geom_ribbon(data=data.pred, #predicted data
                                      aes(x=final_total_min_cw2, y=y.pred,ymin=y.lower,
                   ...
                   ggsave(filename="ModH2_Within.png",
                          width=6, height=4, units='in', dpi=300)  
                   
                   
#Model H2 (Between)
total_minutes_h2_between<-round(seq(min(mspss_analysis$final_total_min,na.rm = TRUE),
                                    max(mspss_analysis$final_total_min,na.rm = TRUE),by=500),2)
                 data.pred = data.frame(final_total_min=total_minutes_h2_between)
                 y.pred = predict(mod_h2_between,
                     newdata=data.pred,se=T,level = 0)
                 data.pred = cbind.data.frame(data.pred,
                                              y.pred)
                 #alternate option
                 scale = 1.96
                  designmat<-model.matrix(eval(eval(mod_h2_between$call$fixed)[-2]),
                        data.pred[-3]) #make design matrix
                  SDvalue<-sqrt(diag(designmat %*% mod_h2_between$varFix %*% t(designmat))) #calculate standard deviation for each point for each model
                  y.lower<-y.pred-(scale*SDvalue) #calculate confidence intervals - lower
                  y.upper<-y.pred+(scale*SDvalue) #calculate confidence intervals - upper
                  data.pred = cbind.data.frame(data.pred,
                             y.lower,
                ...
                data.pred$ypred<-data.pred$y.pred
                        ...
                        ggplot(data=NULL
                          ...
                          ggtitle("Model H2 (Within)")+
                          geom_point(data=mspss_analysis,
                                     aes(x=final_total_min, y=mspss_total,group=record_id,colour=wave),
                                     size=1.5,
                                     alpha=0.5) +
                          geom_line(data=data.pred, #predicted data
                                    aes(x=final_total_min, y=y.pred), 
                                    size=1,
                                    colour="purple2")+
                          geom_ribbon(data=data.pred, #predicted data
                                      aes(x=final_total_min, y=y.pred,ymin=y.lower,
                   ...
                   ggsave(filename="ModH2_Between.png",
                          width=6, height=4, units='in', dpi=300)  
...
standardized_analysis_data <- standardized_analysis_data %>% group_by(record_id) %>% 
  mutate(
    ...
    subj_screen_total_mean2 = mean(subj_screen_total, na.rm = T),
    tech_use_3_mean2 = mean(tech_use_3, na.rm = T)
  ...
  mutate(
    ...
    subj_screen_total_c2 = scale(subj_screen_total, center = T, scale = F),
    tech_use_3_c2 = scale(tech_use_3, center = T, scale = F),
    ...
    subj_screen_total_cw2 = subj_screen_total - subj_screen_total_mean2,
    tech_use_3_cw2 = tech_use_3 - tech_use_3_mean2,
    ...
    tech_use_3_cb2 = tech_use_3_c2 - tech_use_3_cw2
  )

save(standardized_analysis_data, file="Standardized_Analysis_Data.Rda")
```
...
data = standardized_analysis_data, geom = "line", main= "Anxiety (Within Participants)",xlab="Wave", ylab="Centered YAM Score") + theme_classic() + theme(legend.position="none")
...
data = standardized_analysis_data, geom = "line", main= "Social Support (Within Participants)",xlab="Wave", ylab="Centered MSPSS Score") + theme_classic() + theme(legend.position="none")
...
data = standardized_analysis_data, geom = "line", main= "Social Proportion (Within Participants)",xlab="Wave", ylab="Centered Social Proportion") + theme_classic() + theme(legend.position="none")
ggsave("CenteredSocialProportionParticipants.png")

#Centered total min 
qplot(x=wave, y=final_total_min_cw2, colour = factor(record_id),
data = standardized_analysis_data, geom = "line", main= "Total Use (Within Participants)",xlab="Wave", ylab="Centered Total Use") + theme_classic() + theme(legend.position="none")
ggsave("CenteredTotalMinParticipants.png")

#Centered social min 
qplot(x=wave, y=final_social_min_cw2, colour = factor(record_id),
data = standardized_analysis_data, geom = "line", main= "Social Use Min (Within Participants)",xlab="Wave", ylab="Centered Social Use") + theme_classic() + theme(legend.position="none")
...
data = standardized_analysis_data, geom = "line", main= "Friend Stability (Within Participants)",xlab="Wave", ylab="Centered Friend Stability") + theme_classic() + theme(legend.position="none")
...
data = standardized_analysis_data, geom = "line", main= "Anxiety (Between Participants)",xlab="Wave", ylab="Centered YAM Score")  + theme_classic()+ theme(legend.position="none")
#ggsave("CenteredAnxietyParticipants.png")

#Centered social support across waves (by participant)
qplot(x=wave, y=mspss_total_cb2, colour = factor(record_id),
data = standardized_analysis_data, geom = "line", main= "Social Support (Between Participants)",xlab="Wave", ylab="Centered MSPSS Score") + theme_classic() + theme(legend.position="none")
#ggsave("CenteredSocialSupportParticipants.png")

#Centered social use proportion
qplot(x=wave, y=final_social_prop_cb2, colour = factor(record_id),
data = standardized_analysis_data, geom = "line", main= "Social Proportion (Between Participants)",xlab="Wave", ylab="Centered Social Proportion") + theme_classic() + theme(legend.position="none")
#ggsave("CenteredSocialProportionParticipants.png")

#Centered total min 
qplot(x=wave, y=final_total_min_cb2, colour = factor(record_id),
data = standardized_analysis_data, geom = "line", main= "Total Use (Between Participants)",xlab="Wave", ylab="Centered Total Use") + theme_classic() + theme(legend.position="none")
#ggsave("CenteredTotalMinParticipants.png")

#Centered social min 
qplot(x=wave, y=final_social_min_cb2, colour = factor(record_id),
data = standardized_analysis_data, geom = "line", main= "Social Use Min (Between Participants)",xlab="Wave", ylab="Centered Social Use") + theme_classic() + theme(legend.position="none")
#ggsave("CenteredSocialMinParticipants.png")

#Centered friend stability
#ending up as straight lines, values don't make sense
qplot(x=wave, y=prop_friend_stability_cb2, colour = factor(record_id),
data = standardized_analysis_data, geom = "line", main= "Friend Stability (Between Participants)",xlab="Wave", ylab="Centered Friend Stability") + theme_classic() + theme(legend.position="none")
#ggsave("CenteredFriendStabilityParticipants.png")
...
Other Data Viz
```{r}
#Plot: x = wave, y = participant id, plot wave
#line plot to connect participant ids
#, colour = factor(record_id)

qplot(x=wave, y=record_id, colour = factor(record_id),
data = analysis_data, geom = "line", main= "Participants by Wave",xlab="Wave", ylab="Participant") + theme_classic() + theme(legend.position="none")
ggsave("ParticipantWave.png")

qplot(x=StartDate, y=record_id, colour = factor(record_id),
data = analysis_data, geom = "line", main= "Participants by Date",xlab="Date", ylab="Participant") + theme_classic() + theme(legend.position="none")
ggsave("ParticipantDate.png")


#Kate's version

#Subset to usable data
forty_subjects <-  analysis_data %>% group_by(record_id) %>% filter(n()>1) #
forty_subjects


#Plot for participants by date
study_design_plot<- ggplot(forty_subjects,
                           aes(x=StartDate,
                               y=factor(record_id),
...
ggsave(filename="ParticipantDates.png",
       plot=study_design_plot, width=6, height=5, units='in', dpi=300)

#Plot for participants by waves
study_design_plot_waves<- ggplot(forty_subjects,
                           aes(x=wave,
                               y=factor(record_id),
                                color=factor(record_id))) + 
  geom_point(alpha=1) + 
...
print(study_design_plot_waves)
ggsave(filename="ParticipantWaves.png",
       plot=study_design_plot, width=6, height=5, units='in', dpi=300)
```
...
```{r}
#GENERATE COLORS
pal <- wes_palette("Darjeeling1", 37, type = "continuous")

#SPAGHETTI PLOTS
anxiety_p <- ggplot(data = forty_subjects, aes(x = wave, y = yam_total, group = record_id))
anxiety_p+geom_line(aes(color=as.factor(record_id)))+scale_color_manual(values=pal) + theme_classic() + theme(legend.position="none") + xlab("Wave") + ylab("Anxiety")
ggsave("APS_Anxiety.png")

support_p <- ggplot(data = forty_subjects, aes(x = wave, y = mspss_total, group = record_id))
support_p+geom_line(aes(color=as.factor(record_id)))+scale_color_manual(values=pal) + theme_classic() + theme(legend.position="none") + xlab("Wave") + ylab("Social Support")
ggsave("APS_Support.png")

socialmin_p <- ggplot(data = forty_subjects, aes(x = wave, y = final_social_min, group = record_id))
socialmin_p+geom_line(aes(color=as.factor(record_id)))+scale_color_manual(values=pal) + theme_classic() + theme(legend.position="none") + xlab("Wave") + ylab("Social Screen Use")
ggsave("APS_SocialUse.png")

totalmin_p <- ggplot(data = forty_subjects, aes(x = wave, y = final_total_min, group = record_id))
totalmin_p+geom_line(aes(color=as.factor(record_id)))+scale_color_manual(values=pal) + theme_classic() + theme(legend.position="none") + xlab("Wave") + ylab("Total Screen Use")
ggsave("APS_TotalUse.png")

#Remove last data point per participant for friend stability:
forty_subjects_last_dropped <- forty_subjects %>% 
   select(record_id, wave, prop_friend_stability)  %>%
   group_by(record_id) %>% 
   filter(row_number() <= (n() - 1))

stability_p <- ggplot(data = subset(forty_subjects_last_dropped,wave!=1), aes(x = wave, y = prop_friend_stability, group = record_id))
stability_p+geom_line(aes(color=as.factor(record_id)))+scale_color_manual(values=pal) + theme_classic() + theme(legend.position="none") + xlab("Wave") + ylab("Social Connection Stability")
ggsave("APS_Stability.png")
...
print(study_design_plot)
ggsave(filename="APS_Design.png",
       plot=study_design_plot, width=6, height=5, units='in', dpi=300)
