...
#Convert time to numeric value (i.e. 2:04 to 124 minutes)
  socialuse_last$socialtimemin_last = as.numeric(unlist(lapply(socialuse_last$socialtime, function (x) strsplit(x, ":")[[1]][1])))*60 +
  as.numeric(unlist(lapply(socialuse_last$socialtime, function (x) strsplit(x, ":")[[1]][2])))
...
#Convert tiktok use to minutes
  ...
  data_copy <- left_join(data_copy, tiktok_use_last)
   ...
   data_copy <- left_join(data_copy, socialuse_last)
...
#Add tiktok use to social time, new column for screen time as proportion of total use
  ...
  #compare days represented for this_week and last_week, record which one has more days represented
    #if days represented in this week > days represented in last week
  ...
  data_copy$final_social_min[data_copy$days_represented_this_week < data_copy$days_represented_last_week] <- data_copy$socialtimemin_tik_last
  #Choose which week of social use proportion to use
  data_copy$final_social_prop <- data_copy$social_proportion
  data_copy$final_social_prop[data_copy$days_represented_this_week < data_copy$days_represented_last_week] <- data_copy$social_proportion_last
...
#New column for screen time as proportion of total use:
    processed_data <- processed_data %>%
  transform(social_proportion = socialtimemin/tot_time_min)
...
#Model 1: socially focused screen time & anxiety: yam_total ~ socialtimemin
#Model 2: socially focused screen time & perceived social support: mspss_total ~ socialtimemin
#Model 3: socially focused screen time & stable social connections: prop_friend_stability ~ socialtimemin

fit <- lme(socialtimemin ~ yam_total + mspss_total + prop_friend_stability, random = ~1|record_id, data=processed_data, na.action=na.omit)
