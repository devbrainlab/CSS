...
#grand-mean centering (I think)
standardized_analysis_data[c(12:26)] <- lapply(standardized_analysis_data[c(12:26)], 
                             function(x) c(scale(x, center=TRUE, scale=TRUE)))
describe(standardized_analysis_data)
...
#mutate at columns & create new columns with _scaled at end
standardized_analysis_data <- standardized_analysis_data %>% 
  group_by(record_id) %>%
  mutate_at(.vars = colnames(standardized_analysis_data)[12:29], 
            .funs = list("scaled" = scale(.,na.rm=TRUE))) %>%   #find out how to add na.rm=TRUE, maybe? seems to not be an issue so far
  ungroup()
```

Data Viz for centered data
```{r}
