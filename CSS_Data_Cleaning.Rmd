...
for (kid in unique(record_id)) {   #,'b','c','d'
  splitted <- split(netcanvas_by_record$name[which(record_id==kid)],netcanvas_by_record$wave.x[which(record_id==kid)])
  for(wave_num in 0:9){
     final <- final%>% add_row(record_id=kid, wave=wave_num+1, common_friends=length(intersect(unlist(splitted[wave_num]), unlist(splitted[wave_num+1]))), unique_friends=length(union(unlist(splitted[wave_num]), unlist(splitted[wave_num+1]))))
     print(length(union(unlist(splitted[wave_num]), unlist(splitted[wave_num+1]))))
  }
}

#mutate(prop_friend_stability = )
#final <- final %.%
